        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3 sm:items-center sm:justify-start lg:justify-end w-full">
          <div className="flex items-center gap-2 justify-start lg:justify-end w-full sm:w-auto">
            <div className="flex items-center rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
              ...
              <div className="h-8 w-px bg-slate-200 dark:bg-slate-700" aria-hidden="true"></div>
              <button onClick={onNext} className="px-3 py-2 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors" title="Día siguiente">
            ...
            <button 
              onClick={onToday}
              className="px-4 py-2 rounded-full bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-sm font-semibold shadow-sm hover:bg-slate-800 transition-colors"
            >
              Hoy
            </button>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 justify-start lg:justify-end w-full sm:w-auto">
            <div className="inline-flex items-center rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm">
              <button
                onClick={() => setView('day')}
                className={`px-3 py-2 text-sm font-medium rounded-full transition-colors ${view === 'day' ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 shadow' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
              >
                Día
              </button>
              <button
                onClick={() => setView('week')}
                className={`px-3 py-2 text-sm font-medium rounded-full transition-colors ${view === 'week' ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 shadow' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
              >
                Semana
              </button>
            </div>

            <button onClick={onRefresh} className="p-2 rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800" title="Actualizar">
              <RefreshCw size={18} />
            </button>
